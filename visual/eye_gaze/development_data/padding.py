...
for ID in development_set_IDs:
    print(ID, end='\r')
    location = '/data/chercheurs/qureshi191/preprocessed_data/development_data/visual/eye_gaze/individual/'+str(ID)+'_eye_gaze.npy'
    a = np.load(location).T
    X.append(sequence.pad_sequences(a, maxlen=6500, dtype='float32', padding='pre').T.tolist())

X = np.array(X)

np.save('/data/chercheurs/qureshi191/preprocessed_data/development_data/visual/eye_gaze/eye_gaze.npy', X)