...
for ID in test_set_IDs:
    print(ID, end='\r')
    location = '/data/chercheurs/qureshi191/preprocessed_data/test_data/visual/head_pose/individual/'+str(ID)+'_head_pose.npy'
    a = np.load(location).T
    X.append(sequence.pad_sequences(a, maxlen=6500, dtype='float32', padding='pre').T.tolist())

X = np.array(X)

np.save('/data/chercheurs/qureshi191/preprocessed_data/test_data/visual/head_pose/head_pose.npy', X)