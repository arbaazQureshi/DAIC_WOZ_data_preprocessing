...
for ID in test_set_IDs:
    print(ID, end='\r')
    location = '/data/chercheurs/qureshi191/preprocessed_data/test_data/visual/facial_landmarks/individual/'+str(ID)+'_facial_landmarks.npy'
    a = np.load(location).T
    X.append(sequence.pad_sequences(a, maxlen=10000, dtype='float32', padding='pre').T.tolist())

X = np.array(X)

np.save('/data/chercheurs/qureshi191/preprocessed_data/test_data/visual/facial_landmarks/facial_landmarks.npy', X)