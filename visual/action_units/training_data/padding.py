...
for ID in training_set_IDs:
    print(ID, end='\r')
    location = '/data/chercheurs/qureshi191/preprocessed_data/training_data/visual/action_units/individual/'+str(ID)+'_action_units.npy'
    a = np.load(location).T
    X.append(sequence.pad_sequences(a, maxlen=6300, dtype='float32', padding='pre').T.tolist())

X = np.array(X)

np.save('/data/chercheurs/qureshi191/preprocessed_data/training_data/visual/action_units/action_units.npy', X)